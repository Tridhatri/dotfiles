...
vim.opt.expandtab = true vim.opt.tabstop = 2
...
vim.cmd([[
	nnoremap g= g+| " g=g=g= is apparently less awkward than g+g+g+
...
map({ "n" }, "<leader><leader>", "gt", { desc = "Next tab" })
...
map({ "n" }, "<C-j>", "<C-w>j", { desc = "Navigate to bottom split" })
map({ "n" }, "<C-k>", "<C-w>k", { desc = "Navigate to top split" })
